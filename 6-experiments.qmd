...
-  A treatment (or exposure) is often represented as a binary variable, typically coded as 1 if an individual receives the treatment and 0 if they do not (the control condition). Treatments can also be multi-valued or continuous, but the binary case is the simplest and most common starting point.

-   For each individual or observational unit, we measure an outcome Y, which may be continuous (e.g.,  blood pressure) or categorical (e.g., disease status).

- A treatment has a causal effect for an individual if their outcome under treatment differs from their outcome under control. 

- The key challenge, often called the fundamental problem of causal inference, is that for any individual, we can observe only one of these potential outcomes, the other potential outcome is the counterfactual. 

- Because we cannot observe individual-level causal effects directly, we instead estimate average treatment effects (ATE) by comparing groups. 
  ...
  -   The group comparison is only valid if treatment assignment is independent of potential outcomes, for this to work we must rely on randomization.
...
**Randomization.** Subjects should be randomly selected from the population (where possible) and randomly divided into treatment groups to avoid unintentional selection bias in the groups and to account for variables that cannot be controlled. For example, some patients may be inherently more susceptible to a disease than others and we wouldn't want all of those patients to end up in the same group. 
...
**Exercise** It’s natural to wonder if there is a difference in tooth length by supplement type. Use `group_by` and `summarise` to explore this and create an appropriate visualization using `ggplot`.
...
2.  A field with a fertility gradient

![](images/field1.png){width="400"}

3.  A greenhouse with a temperature gradient during the day
...
**Exercise** Use `group_by` and `summarise` to look at the average yield within each variety.

```{r, eval = FALSE}
oatvar %>% 
```

**Exercise** Look at the results of using `lm` with variety and block as predictors.
