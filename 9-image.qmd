...
mnist_for_plotting <- readRDS("data/mnist_for_plotting.rds")
...
```{r}
as_tibble(mnist_for_plotting)
```

Then we can plot using standard ggplot methods. For example

```{r}
ggplot(mnist_for_plotting, aes(x = x, y = y, fill = pixel_intensity)) +
  geom_tile() +
