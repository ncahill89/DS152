...
The `gcfeeling` data contains results of a survey conducted by Georgetown College on 47  students in relation to feelings about the college.
...
```{r}
gcfeeling %>% 
  summarise(corr= cor(rating.fresh, rating.diff))
```
...
# Boxplots grouped by a categorical variable
  ggplot(gcfeeling, aes(x = athlete, y = rating.diff)) +
  geom_boxplot() +
  theme_minimal()
```


#### **Scatter Plots of Numerical Variables by Categories**

We can visualise the relationship between two numerical variables and a categorical variable, for example, freshman happiness, happiness difference and athlete categories.

```{r}
ggplot(gcfeeling, aes(x = rating.fresh, y = rating.diff, colour = athlete)) +
  geom_point()
```
...
```{r}
mod_lm <- lm(rating.diff ~ rating.fresh + athlete, data = gcfeeling)
mod_lm %>% coef()
```

#### **Binary outcome**

We can use multiple logistic regression to model student happiness when using a binary outcome.

```{r}
mod_glm <- glm(happier ~ rating.fresh + athlete, data = gcfeeling, family = "binomial")
