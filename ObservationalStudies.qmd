...
If `gcfeeling` contains categorical variables, we can visualize their distributions:
...
cor(gcfeeling %>% select_if(is.numeric), use = "complete.obs")
```
...
gcfeeling %>%
  ggplot(aes(x = factor(groupvar), y = numeric_var, fill = factor(groupvar))) +
  geom_boxplot() +
  theme_minimal()
```
